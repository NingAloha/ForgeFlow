...
pub fn is_candidate_complete(domain: &str, problem: &str, capability: &str) -> bool {
    !is_blank(domain) && !is_blank(problem) && !is_blank(capability)
}
...
pub fn build_boundary_preview(domain: &str, problem: &str, capability: &str) -> String {
    if is_blank(domain) {
    ...
    if is_blank(capability) {
        return format!(
            "已识别领域：{}\n已识别核心问题：{}\n我还无法确定这个项目解决核心问题所依赖的核心能力。\n请补充：这个项目主要依靠什么核心能力来解决上述问题？",
            domain, problem
        );
    }
...
mod tests {
    ...
    #[test]
    fn preview_enters_confirmation_when_domain_and_problem_present() {
        ...
        assert!(text.contains("3. 核心能力：通过自然语言辅助软件开发"));
    }
    ...
    #[test]
    fn preview_prompts_missing_capability_when_capability_is_blank() {
        let text = build_boundary_preview("软件开发工具", "上下文切换频繁导致效率下降", "  ");
        assert!(text.contains("已识别领域：软件开发工具"));
        assert!(text.contains("已识别核心问题：上下文切换频繁导致效率下降"));
        assert!(text.contains("请补充：这个项目主要依靠什么核心能力来解决上述问题？"));
        assert!(!text.contains("这些理解是否一致？"));
        assert!(!text.contains("3. 核心能力："));
    }
    ...
    fn incomplete_candidate_is_not_complete() {
        ...
        assert!(!is_candidate_complete("AI工具", "   ", "核心能力"));
        assert!(!is_candidate_complete("   ", "问题", "核心能力"));
        assert!(!is_candidate_complete("   ", "   ", "核心能力"));
        assert!(is_candidate_complete("AI工具", "问题", "核心能力"));
    }
}
