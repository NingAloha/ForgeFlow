...
PROMPT_PATH = Path(__file__).parent / "prompts" / "target_users_system.txt"

TARGET_USERS_ALLOWED_PATHS: set[tuple[str, ...]] = {
...
def update_target_users(
    ...
    validate_allowed_mutation(
        ...
        allowed_paths=TARGET_USERS_ALLOWED_PATHS,
    )

    return proposed_artifact


def _find_target_user_question(artifact: dict[str, Any]) -> str:
    ...
    for question in open_questions:
        ...
        if "目标用户" in question or "用户是谁" in question:
            return question

    raise RuntimeError("no target-user open question found")
...
def main() -> None:
    ...
    current_question = _find_target_user_question(artifact)

    print(f"Current question:\n{current_question}")

    user_answer = input("\nAnswer> ").strip()

    updated_artifact = update_target_users(
