...
PROMPT_PATH = Path(__file__).parent / "prompts" / "application_boundary_system.txt"

APPLICATION_BOUNDARY_ALLOWED_PATHS: set[tuple[str, ...]] = {
...
def update_application_boundary(
    ...
    validate_allowed_mutation(
        ...
        allowed_paths=APPLICATION_BOUNDARY_ALLOWED_PATHS,
    )

    return proposed_artifact


def _find_application_boundary_question(artifact: dict[str, Any]) -> str:
    ...
    raise RuntimeError("no application-boundary open question found")
...
def main() -> None:
    ...
    current_question = _find_application_boundary_question(artifact)

    print(f"Current question:\n{current_question}")

    user_answer = input("\nAnswer> ").strip()

    updated_artifact = update_application_boundary(
