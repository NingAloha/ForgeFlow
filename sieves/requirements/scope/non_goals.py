...
PROMPT_PATH = Path(__file__).parent / "prompts" / "non_goals_system.txt"

NON_GOALS_ALLOWED_PATHS: set[tuple[str, ...]] = {
    ("scope", "non_goals"),
    ("open_questions",),
}


def update_non_goals(
    ...
    validate_allowed_mutation(
        ...
        allowed_paths=NON_GOALS_ALLOWED_PATHS,
    )

    return proposed_artifact


def _find_non_goals_question(artifact: dict[str, Any]) -> str:
    ...
    for question in open_questions:
        ...
        if "不做" in question or "暂不支持" in question or "排除" in question:
            return question

    raise RuntimeError("no non-goals open question found")
...
def main() -> None:
    ...
    current_question = _find_non_goals_question(artifact)

    print(f"Current question:\n{current_question}")

    user_answer = input("\nAnswer> ").strip()

    updated_artifact = update_non_goals(
