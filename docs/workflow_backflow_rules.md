...
| --- | --- | --- | --- | --- |
| `TESTING` | `test_report.json` | `result = not_run`，或 `partial` 但归因不足 | `IMPLEMENTATION` | 问题主要是模块实现错误，`related_modules` 可定位，contract 本身仍成立 |
| `TESTING` | `test_report.json` + `system_design.json` | 不适用 | `DESIGN` | `related_contracts` 指向 contract / data flow 缺陷，多模块边界失败 |
| `TESTING` | `test_report.json` + `solution.json` | 不适用 | `SOLUTION` | 模块职责、承接关系或关键技术主干不成立 |
| `TESTING` | `test_report.json` + `spec.json` | 不适用 | `REQUIREMENTS` | 验收口径、硬约束或需求目标本身不稳定 |
| `IMPLEMENTATION` | `implementation_status.json` | `in_progress`，或 `blocked` 但只是环境 / 工具 / 依赖阻塞 | `DESIGN` | contract、data flow、project structure 不足以指导当前实现 |
| `IMPLEMENTATION` | `implementation_status.json` + `solution.json` | 不适用 | `SOLUTION` | 模块职责、核心承接关系或技术路线错误 |
| `IMPLEMENTATION` | `implementation_status.json` + `spec.json` | 不适用 | `REQUIREMENTS` | 需求目标、约束或验收标准不足以支撑继续编码 |
| `DESIGN` | `system_design.json` | design 仍可补完，未出现上游失效 | `SOLUTION` | solution 无法落成稳定的结构、contract 或 flow |
| `DESIGN` | `system_design.json` + `spec.json` | 不适用 | `REQUIREMENTS` | 需求目标、范围、验收口径或约束不稳，导致 design 无法收敛 |
| `SOLUTION` | `solution.json` + `spec.json` | 方案仍可收紧，主干未失效 | `REQUIREMENTS` | 需求目标、约束、验收标准或 MVP 范围不稳，导致方案无法收敛 |

## `TESTING -> IMPLEMENTATION / DESIGN / SOLUTION / REQUIREMENTS`
...
* `related_contracts` 非空不自动等于回 `DESIGN`；如果 contract 定义本身成立，只是实现未遵守，仍应优先回 `IMPLEMENTATION`。
...
## `IMPLEMENTATION -> DESIGN / SOLUTION / REQUIREMENTS`

在当前单线程主流程中，`IMPLEMENTATION` 阶段的主判断输入是 `state/implementation_status.json`。orchestrator 应优先读取其中的 `module_name`、`implementation_status`、`contract_compliance`、`known_limitations` 与 `blockers`，再结合 `state/system_design.json`、`state/solution.json` 与 `state/spec.json` 判断当前阻塞是否仍可在实现层解决。

应维持在 `IMPLEMENTATION`，而不是立即回流的情况：
...
在当前单线程主流程中，`DESIGN` 阶段的主判断输入是 `state/system_design.json`。orchestrator 应优先读取其中的 `project_structure`、`contracts`、`data_flow` 与 `mvp_plan`，再结合 `state/solution.json` 与 `state/spec.json` 判断当前 design 是否只是未补完，还是已经暴露出上游前提不足。

应维持在 `DESIGN`，而不是立即回流的情况：
...
* 不先调整 solution，`system_design.json` 无法补成 `DESIGN` 所要求的可实现结构。
...
在当前单线程主流程中，`SOLUTION` 阶段的主判断输入是 `state/solution.json`。orchestrator 应优先读取其中的 `selected_stack`、`module_mapping`、`risks` 与 `alternatives`，再结合 `state/spec.json` 判断当前方案是尚未收紧，还是已经暴露出 requirements 前提不成立。

应维持在 `SOLUTION`，而不是立即回流的情况：
...
* 只有当新的 requirements 再次满足 `REQUIREMENTS` 判定条件后，下游阶段才应重新建立。
