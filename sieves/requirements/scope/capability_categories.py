...
PROMPT_PATH = Path(__file__).parent / "prompts" / "capability_categories_system.txt"

CAPABILITY_CATEGORIES_ALLOWED_PATHS: set[tuple[str, ...]] = {
    ("scope", "capability_categories"),
    ("open_questions",),
}


def update_capability_categories(
    ...
    validate_allowed_mutation(
        ...
        allowed_paths=CAPABILITY_CATEGORIES_ALLOWED_PATHS,
    )

    return proposed_artifact


def _find_capability_categories_question(artifact: dict[str, Any]) -> str:
    ...
    for question in open_questions:
        ...
        if "核心能力类别" in question or "能力类别" in question:
            return question

    raise RuntimeError("no capability-categories open question found")
...
def main() -> None:
    ...
    current_question = _find_capability_categories_question(artifact)

    print(f"Current question:\n{current_question}")

    user_answer = input("\nAnswer> ").strip()

    updated_artifact = update_capability_categories(
